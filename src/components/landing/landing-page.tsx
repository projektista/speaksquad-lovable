...
export function LandingPage({ content, lang }: { content: LandingContent; lang: "pt" | "jp" }) {
  return (
      ...
      <div className="relative mx-auto grid max-w-6xl grid-cols-1 items-center gap-10 px-5 pb-16 pt-14 md:grid-cols-[1.2fr_0.8fr] md:pb-24 md:pt-20">
        <div>
          ...
          <div className="mt-8 flex flex-wrap items-center gap-3">
            <Link to={signupPath} className="btn-primary btn-shine px-7 py-3.5 text-lg md:text-xl">
              {hero.ctaPrimary}
            </Link>
          </div>
...
function Metodo({ content }: { content: LandingContent }) {
  ...
  return (
    <section id="method" className="section-glow-cyan bg-grid-parallax relative">
...
function Fluxo({ content }: { content: LandingContent }) {
  const { fluxo } = content;
  return (
    <section id="flow" className="section-glow-violet bg-grid-parallax relative">
      <div className="mx-auto max-w-6xl px-5 py-20">
        <Reveal>
...
function Pricing({ content }: { content: LandingContent }) {
  ...
  return (
    <section id="pricing" className="section-glow-magenta bg-grid-parallax relative">
...
function FAQ({ content }: { content: LandingContent }) {
  ...
  const categories = faq.categories
    .map((cat, ci) => ({
      ...
      items: q
        ? cat.items.filter(
            (f) => f.q.toLowerCase().includes(q) || answerToText(f.a).toLowerCase().includes(q),
          )
        : cat.items,
    }))
  ...
  return (
    <section id="faq" className="section-glow-magenta bg-grid-parallax relative">
      <script
    ...
    <>
      {parts.map((part, i) =>
        part.toLowerCase() === term.toLowerCase() ? (
          <mark key={i} className="rounded bg-cyan/25 px-0.5 text-cyan">
            {part}
          </mark>
        ) : (
    ...
    <>
      {answer.map((part, i) =>
        typeof part === "string" ? (
          ...
          <Link key={i} to={part.to} className="text-cyan underline underline-offset-4">
